https://zhuanlan.zhihu.com/p/62509258
...
常见的ANR有service、broadcast、provider以及input，更多细节详见理解Android ANR的触发原理，[http://gityuan.com/2016/07/02/android-anr](https://link.zhihu.com/?target=http%3A//gityuan.com/2016/07/02/android-anr)，接下来本文以图文形式分别讲解。

### service超时机制

下面来看看埋炸弹与拆炸弹在整个服务启动(startService)过程所处的环节。

![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmxqt61xhwj30qo0k075r.jpg)
...
更多细节详见startService启动过程分析，[http://gityuan.com/2016/03/06/start-service](https://link.zhihu.com/?target=http%3A//gityuan.com/2016/03/06/start-service)

### broadcast超时机制

broadcast跟service超时机制大抵相同，对于静态注册的广播在超时检测过程需要检测SP，如下图所示。

![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmxqt4o1hxj30qo0k0gna.jpg)
...
![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmxqt2qjznj30qo0k0401.jpg)
...
更多细节详见Android Broadcast广播机制分析，[http://gityuan.com/2016/06/04/broadcast-receiver](https://link.zhihu.com/?target=http%3A//gityuan.com/2016/06/04/broadcast-receiver)

### provider超时机制

provider的超时是在provider进程首次启动的时候才会检测，当provider进程已启动的场景，再次请求provider并不会触发provider超时。

![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmxqt1i2ayj30qo0k0760.jpg)
...
更多细节详见理解ContentProvider原理，[http://gityuan.com/2016/07/30/content-provider](https://link.zhihu.com/?target=http%3A//gityuan.com/2016/07/30/content-provider)
...
![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmxqszr8udj30qk0ksq4r.jpg)
...
1. 当应用窗口准备就绪，则将mPendingEvent转移到outBoundQueue队列
2. 当outBoundQueue不为空，且应用管道对端连接状态正常，则将数据从outboundQueue中取出事件，放入waitQueue队列
3. InputDispatcher通过socket告知目标应用所在进程可以准备开始干活
4. App在初始化时默认已创建跟中控系统双向通信的socketpair，此时App的包工头(main线程)收到输入事件后，会层层转发到目标窗口来处理
5. 包工头完成工作后，会通过socket向中控系统汇报工作完成，则中控系统会将该事件从waitQueue队列中移除。
...
更多细节详见Input系统-ANR原理分析，[http://gityuan.com/2017/01/01/input-anr](https://link.zhihu.com/?target=http%3A//gityuan.com/2017/01/01/input-anr)

### ANR超时阈值

不同组件的超时阈值各有不同，关于service、broadcast、contentprovider以及input的超时阈值如下表：

![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmxqt9bqotj30pz0bl74x.jpg)
...
![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmxqtbrzlxj30k20hrq5a.jpg)

关于细节详见解读Android进程优先级ADJ算法，[http://gityuan.com/2018/05/19/android-process-adj](https://link.zhihu.com/?target=http%3A//gityuan.com/2018/05/19/android-process-adj)
