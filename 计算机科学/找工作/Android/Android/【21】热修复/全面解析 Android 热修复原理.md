...
这是【Android 修炼手册】第 8 篇文章，如果还没有看过前面系列文章，欢迎点击 [这里](https://link.zhihu.com/?target=https%3A//juejin.im/post/5d3c28e8e51d45109b01b293) 查看～
...
1. clone [CommonTec](https://link.zhihu.com/?target=https%3A//github.com/5A59/android-training/tree/master/common-tec/CommonTec) 项目，其中 hotfix 和 patch 是热修复代码 示例代码基于 AndFix，NuWa，Robust 进行了调整，抽取主要部分用来讲解原理。

### 文章概览



![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmylqi4mn7j30u01anq7x.jpg)



### 一、热修复和插件化

插件化和热修复的原理，都是动态加载 dex／apk 中的类／资源，两者的目的不同。插件化目标在于加载 activity 等组件，达到动态下发组件的功能，热修复目标在修复已有的问题。目标不同，也就导致其实现方式上的差别。由于目标是动态加载组件，所以插件化重在解决组件的生命周期，以及资源的问题。而热修复重在解决替换已有的有问题的类／方法／资源等。 关于插件化，可以看前面分享的文章[Android 插件化分析](https://link.zhihu.com/?target=https%3A//juejin.im/post/5d235f8b51882554c007af06)

### 二、使用 gradle 简化插件开发流程

如果看过[Android 插件化分析](https://link.zhihu.com/?target=https%3A//juejin.im/post/5d235f8b51882554c007af06)里的 gradle 简化插件开发流程，这里可以略过～
...
如果看过[Android 插件化分析](https://link.zhihu.com/?target=https%3A//juejin.im/post/5d235f8b51882554c007af06)里的 ClassLoader 分析，这里可以略过～
...
在这里，我们统称 dex 文件，包含 dex 的 apk 文件以及 jar 文件为 dex 文件 PathClassLoader 用来加载系统类和应用程序类，用来加载 dex 文件，但是 dex2oat 生成的 odex 文件只能放在系统的默认目录。
...
Qzone／NuwaAndFixRobustTinkerSophixdex 修复yyyyyso 修复nnnyy资源修复nnnyy全平台支持ynyyy即时生效nyyn同时支持补丁包大小大小小小小
...
![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmylqe6w09j311y0khwgd.jpg)



这里也顺便说一下上面三个结构的内容是什么时候填充的，就是在 ClassLoader 加载类的时候。简图如下：

![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmylqced07j30u012jdlm.jpg)
...
![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmylqazt2nj30pe0fswfu.jpg)



所以 native hook 的本质就是把旧方法的 ArtMethod 内容替换成新方法的 ArtMethod 内容。 具体的实现代码在[这里](https://link.zhihu.com/?target=https%3A//github.com/5A59/android-training/blob/master/common-tec/CommonTec/hotfix/src/main/java/com/zy/hotfix/native_hook/NativeHookUtils.kt)(只实现了 Android N 上的修复)，下面看一些重点代码。
...
![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmylq929icj30w20immyu.jpg)
...
![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmylq78nkij30w20imq4l.jpg)



具体的实现在[这里](https://link.zhihu.com/?target=https%3A//github.com/5A59/android-training/blob/master/common-tec/CommonTec/hotfix/src/main/java/com/zy/hotfix/insert_dex/InsertDexUtils.java)，下面放一些重点代码。
...
2. class verify 问题。关于这个问题可以看[Qzone 的解释](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzI1MTA1MzM2Nw%3D%3D%26mid%3D400118620%26idx%3D1%26sn%3Db4fdd5055731290eef12ad0d17f39d4a)，这里就不详细展开了
...
InstantRun 是 AndroidStudio 2.0 新增的功能，方便快速的增量编译应用并部署，美团参照其原理实现了 Robust 热修复框架。 其中的原理是，给每个 Class 中新增一个 changeQuickRedirect 的静态变量，并在每个方法执行之前，对这个变量进行了判断，如果这个变量被赋值了，就调用补丁类中的方法，如果没有被赋值，还是调用旧方法。 原理比较简单，下面看看实现。具体实现在[这里](https://link.zhihu.com/?target=https%3A//github.com/5A59/android-training/blob/master/common-tec/CommonTec/hotfix/src/main/java/com/zy/hotfix/instant_run/InstantRunUtils.java)。

![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmylq5f24gj31al0u046r.jpg)
...
![img](https://tva1.sinaimg.cn/large/008eGmZEly1gmylq2qzvaj30u017swn7.jpg)



### 参考资料

[https://www.cnblogs.com/popfisher/p/8543973.html](https://link.zhihu.com/?target=https%3A//www.cnblogs.com/popfisher/p/8543973.html)[https://tech.meituan.com/2016/09/14/android-robust.html](https://link.zhihu.com/?target=https%3A//tech.meituan.com/2016/09/14/android-robust.html)
[深入探索Android热修复技术原理](https://link.zhihu.com/?target=https%3A//yq.aliyun.com/articles/115122%3Fspm%3Da2c4g.11186623.2.31.38ec5c56o0QqEx)
