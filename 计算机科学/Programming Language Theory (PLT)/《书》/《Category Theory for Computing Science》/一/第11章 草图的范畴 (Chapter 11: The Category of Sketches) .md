...
在计算机科学中，**参数化数据类型（Parametrized Data Types）** 是指可以接受一个或多个类型参数的数据类型，如列表（`List<T>`）、树（`Tree<T>`）等。使用图式同态，我们可以形式化地定义和操作这些数据类型，通过范畴论中的**推广（Pushouts）** 构造参数化数据类型。
...
- **类型理论**：通过图式同态和参数化数据类型的构造，形式化编程语言中的类型系统。
