...
在本章中，我们将看到一种计算效应——**可变引用**（mutable references）——如何添加到我们已经研究的演算中。主要的扩展将是显式地处理存储器（store，也称为堆 heap）。这个扩展的定义是直接的；最有趣的部分是我们需要对类型保持定理（type preservation theorem，13.5.3）进行🥑细化。我们将在第14章讨论另一种效应——异常（exceptions）和非本地控制转移。
