

```
请先从宏观角度帮助我理解整本书和每一章讲了什么。
然后再从微观角度讲解每一章每一节分别都讲了什么。
它们之间有什么关系和联系，有什么承上启下的关系。
在函数式编程和PLT编程语言设计中有什么关系和应用。
```





```
我正在阅读： Philip Wadler 和 Wen Kokke 所著的《Programming Language Foundations in Agda》的中文翻译
帮我详解其内容，重点，作用，通俗解释，在函数式编程和PLT编程语言设计中有什么关系和应用
这些agda代码都很难懂。请为他们加上注释。
然后我是一个没有任何背景知识的小白，所以你讲解的时候应该照顾我的感受。
并且详解，不要省略任何。然后再结尾和开头添加一些整体性的解释概览帮助我理解。
在函数式编程和PLT编程语言设计中有什么关系和应用
```



```
讲解每一章每一节分别都讲了什么的时候，按照以下框架：
内容，重点，作用，通俗解释，在函数式编程和PLT编程语言设计中有什么关系和应用
重新生成。
```



```
《Programming Language Foundations in Agda》
第一分册：逻辑基础
1. Naturals: 自然数
2. Induction: 归纳证明
3. Relations: 关系的归纳定义
4. Equality: 相等性与等式推理
5. Isomorphism: 同构与嵌入
6. Connectives: 合取、析取与蕴涵
7. Negation: 直觉逻辑与命题逻辑中的否定
8. Quantifiers: 全称量词与存在量词
9. Decidable: 布尔值与判定过程
10. Lists: 列表与高阶函数
第二分册：编程语言基础
1. Lambda: λ-演算简介
2. Properties: 可进性与保型性
3. DeBruijn: 内在类型的 de Bruijn 表示法
4. More: 简单类型 λ-演算的更多构造
5. Bisimulation: 联系不同的归约系统
6. Inference: 双向类型推理
7. Untyped: 完全正规化的无类型 λ-演算
8. Confluence: 无类型 λ-演算的合流性
9. BigStep: 无类型 λ-演算的大步语义
第三分册：指称语义
1. Denotational: 无类型 λ-演算的指称语义
2. Compositional: 指称语义的可组合性
3. Soundness: 指称语义归约的可靠性
4. Adequacy: 指称语义相对于操作语义的充分性
5. ContextualEquivalence: 指称相等蕴含语境等价
```

