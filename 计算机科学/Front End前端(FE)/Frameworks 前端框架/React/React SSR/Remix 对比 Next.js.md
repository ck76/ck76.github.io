...
**差异性** Remix 路由是基于 react-router 构建的, 并且你可以直接使用 React Hooks, 比如 `useParams` 和 `useNavigate`。 另一方面, Remix 内置支持嵌套布局的嵌套路由，而 Nest.js 则不支持.
...
在 Next.js 中， 开发人员可以自由的选择上述所有方法技术从加载数据，比如我的[博客](https://link.juejin.cn?target=https%3A%2F%2Fmaqib.cn%2F)就是使用了 SSG 将文章详情页打成静态资源，新写的文章就使用了增量静态生成，不需要再次打包。
...
```jsx
jsx
复制代码export const getServerSideProps = async ({ params, query }) => {
  const id = params.id
  // 通过 url 上的id 获取数据
...
```jsx
jsx
复制代码import { useLoaderData } from "remix";
...
```javascript
javascript
复制代码import { createCookie } from "remix";
...
Remix 提供了内置表单功能 通过 `<button name="_action">`代替 `<button name="action">` 然后可以在 action 中, 使用 `formData.get("\_action")` 获取数据

```jsx
jsx
复制代码export async function action({ request }) {
...
- [网站的未来：Next.js 与 Remix](https://link.juejin.cn?target=https%3A%2F%2Fsegmentfault.com%2Fa%2F1190000041050654)
- [Remix vs. Next.js: A Detailed Comparison](https://link.juejin.cn?target=https%3A%2F%2Fblog.bitsrc.io%2Fremix-vs-next-js-a-detailed-comparison-6ff557f7b41f)



作者：狂奔滴小马
链接：https://juejin.cn/post/7067454063708749860
来源：稀土掘金
